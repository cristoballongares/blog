...
Al principio resultaba fácil, supuse que con fuerza bruta lo resolvia, hasta que leí esa ultima restriccion de *b* (Solo imaginemos calcular $20^{2147483000}$). Si lo hacemos asi, la complejidad seria $O(b)$, lo cual NO seria nada optimo.
...
```
> Hay otro codigo un poco mas sencillo y mas interesante!, hace uso de bits:), sin embargo, aun estoy trabajando en el :p, requiere entender de bitmasking.
